...
public class BlockDamaged
  extends Block
  implements ITieredBlock, ISpecialToolTip, ISpecialUnlocalizedName
{
  ...
  public BlockDamaged addBlockList(String blockname, int meta)
  {
    if ((meta < 0) || (meta >= 16))
    {
      ClayiumCore.logger.error("Can't register block " + blockname + " to " + super.getUnlocalizedName() + ". The metadata is invalid.");
      return this;
    }
    if (isRegistered(meta))
    {
      ClayiumCore.logger.error("Can't register block " + blockname + " to " + super.getUnlocalizedName() + ". The metadata has been used.");
      return this;
    }
    if (blockname == null)
    {
      ClayiumCore.logger.error("Can't register block " + blockname + " to " + super.getUnlocalizedName() + ". The blockname is invalid.");
      return this;
    }
    if (getMeta(blockname) != -1)
    {
      ClayiumCore.logger.error("Can't register block " + blockname + " to " + super.getUnlocalizedName() + ". The blockname has been used.");
      return this;
    }
  ...
  {
    if ((getMeta(blockname) < 0) || (getMeta(blockname) >= 16)) {
      return this;
    }
    if (this.hashs[getMeta(blockname)] == null) {
      this.hashs[getMeta(blockname)] = new HashMap();
    }
  ...
  {
    for (int i = 0; i < 16; i++) {
      if ((isRegistered(i)) && (this.displayModes[i] != 0)) {
  ...
  protected String getUnlocalizedName(int meta)
  {
    if (isRegistered(meta))
    {
      ...
      return super.getUnlocalizedName() + "." + this.unlocalizedSubNames[meta];
    }
    return null;
  }

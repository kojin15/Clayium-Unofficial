...
import mods.clayium.util.PlayerKey.KeyType;
  ...
  implements IOverridableBlock, ISpecialToolTip
{
  ...
  public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ)
  {
    ...
    if (!(UtilBuilder.safeGetTileEntity(world, x, y, z) instanceof TileGeneric)) {
      return false;
    }
  ...
  {
    player.openGui(ClayiumCore.INSTANCE, guiId, world, x, y, z);
  }
  
  protected void openGui(World world, int x, int y, int z, EntityPlayer player)
  {
    Block block = world.getBlock(x, y, z);
    if (((block instanceof ClayContainer)) && ((UtilBuilder.safeGetTileEntity(world, x, y, z) instanceof TileGeneric))) {
  ...
  {
    TileEntity tile = UtilBuilder.safeGetTileEntity(world, x, y, z);
    int m = -1;
    if (this.metaMode == 0)
    {
      m = 0;
    ...
    {
      m = BlockPistonBase.determineOrientation(world, x, y, z, entity);
    }
    if (m != -1)
    {
      world.setBlockMetadataWithNotify(x, y, z, m, 2);
      if (tile != null) {
        tile.blockMetadata = m;
      }
    }
    if ((!itemstack.hasDisplayName()) || 
    
      ((world.getTileEntity(x, y, z) instanceof TileGeneric))) {
      ((TileGeneric)world.getTileEntity(x, y, z)).onBlockPlacedBy(world, x, y, z, entity, itemstack);
    }
  }
  
  protected ForgeDirection[][] validRotations = { new ForgeDirection[0], { ForgeDirection.UP, ForgeDirection.DOWN }, ForgeDirection.VALID_DIRECTIONS };
  ...
  public boolean rotateBlock(World worldObj, int x, int y, int z, ForgeDirection axis)
  {
    ...
    TileEntity tile = UtilBuilder.safeGetTileEntity(worldObj, x, y, z);
  ...
  public ArrayList<ItemStack> getDrops(World world, int x, int y, int z, int metadata, int fortune)
  {
    ...
    ArrayList<ItemStack> res;
    if (((UtilBuilder.safeGetTileEntity(world, x, y, z) instanceof TileGeneric)) && 
      (((TileGeneric)UtilBuilder.safeGetTileEntity(world, x, y, z)).hasSpecialDrops())) {
      res = ((TileGeneric)UtilBuilder.safeGetTileEntity(world, x, y, z)).getDrops(world, x, y, z, this, metadata, fortune);
    } else {
      res = super.getDrops(world, x, y, z, metadata, fortune);
    }
    if (((UtilBuilder.safeGetTileEntity(world, x, y, z) instanceof TileGeneric)) && 
      (((TileGeneric)UtilBuilder.safeGetTileEntity(world, x, y, z)).getRemoveFlag())) {
      world.removeTileEntity(x, y, z);
    }
    return res;
  }
  ...
  {
    return meta < 6 ? meta : this.metaMode == 0 ? UtilDirection.NORTH.ordinal() : meta - 6;
  }
  ...
  public MovingObjectPosition collisionRayTrace(World world, int x, int y, int z, Vec3 start, Vec3 end)
  {
    ...
    for (UtilDirection direction : directions) {
      if (checkPipeConnection(world, x, y, z, direction))
      {
        ...
        {
          res = pos;mindirection = direction;
        }
      }
    }
    setBlockBounds((float)(0.5D - o), (float)(0.5D - o), (float)(0.5D - o), (float)(0.5D + o), (float)(0.5D + o), (float)(0.5D + o));
  ...
  public static int getConnectionAsImport(TileEntity tile, UtilDirection from, TileEntity tile2)
  {
    ...
    {
      if (((tile instanceof IEnergyConnection)) && (((IEnergyConnection)tile).canConnectEnergy(from.toForgeDirection())) && ((tile2 instanceof IEnergyConnection)) && 
        (((IEnergyConnection)tile2).canConnectEnergy(from.getOpposite().toForgeDirection()))) {
        return 2;
      }
      ...
      if (((container instanceof TileFluidTranslator)) && ((tile2 instanceof IFluidHandler))) {
        if (!(tile2 instanceof TileFluidTranslator)) {
          return 2;
        }
      }
  ...
  public static int getConnectionAsExport(TileEntity tile, UtilDirection to, TileEntity tile2)
  {
    if ((tile instanceof IInventory))
    {
      if (((tile instanceof IEnergyConnection)) && (((IEnergyConnection)tile).canConnectEnergy(to.toForgeDirection())) && ((tile2 instanceof IEnergyConnection)) && 
        (((IEnergyConnection)tile2).canConnectEnergy(to.getOpposite().toForgeDirection()))) {
        return 2;
      }
      if (!(tile instanceof TileClayContainer)) {
        return 1;
  ...
  public void addCollisionBoxesToList(World world, int x, int y, int z, AxisAlignedBB aabb, List list, Entity p_149743_7_)
  {
    ...
    else
    {
      ...
      for (UtilDirection direction : directions)
      {
        TileEntity te = direction.getTileEntity(world, x, y, z);

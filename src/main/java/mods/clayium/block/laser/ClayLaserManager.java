...
public class ClayLaserManager
{
  ...
  public void irradiateLaser()
  {
    ...
    int x = this.x;int dx = this.direction.offsetX;
    int y = this.y;int dy = this.direction.offsetY;
    int z = this.z;int dz = this.direction.offsetZ;
    ...
    for (int i = 1; i < laserLengthMax; i++)
    {
      ...
      {
        block = this.world.getBlock(xx, yy, zz);
    ...
    if (this.hasTarget)
    {
      long n = this.clayLaser.getEnergy();
      ...
      for (int i = 0; i < this.laserEnergyHistory.length; i++) {
        if (i < this.laserEnergyHistory.length - 1) {
          this.laserEnergyHistory[i] = this.laserEnergyHistory[(i + 1)];
        } else {
          this.laserEnergyHistory[i] = n;
        }
      }
      Block block = this.world.getBlock(this.xx, this.yy, this.zz);
      int meta = this.world.getBlockMetadata(this.xx, this.yy, this.zz);
      TileEntity tile = UtilBuilder.safeGetTileEntity(this.world, this.xx, this.yy, this.zz);
      if ((block.getMaterial() == Material.rock) && (block != Blocks.bedrock) && (this.totalIrradiatedEnergy >= (block.getBlockHardness(this.world, this.xx, this.yy, this.zz) + 1.0F) * 100L))
      {
  ...
  {
    return this.hasTarget ? new int[] { this.xx, this.yy, this.zz } : null;
  }
  ...
  {
    List<ItemStack> itemToDrop = new ArrayList();
    ...
    boolean canSilkHarvest = false;int fortune = 0;
    ItemStack itemstack;
    float g;
    if (canSilkHarvest)
    {
    ...
    if (dropXP) {
      block.dropXpOnBlockBreak(theWorld, xx, yy, zz, block.getExpDrop(theWorld, l, fortune));
    }
    return (ItemStack[])itemToDrop.toArray(new ItemStack[0]);
  }
  ...
  public void dropItems(World theWorld, int xx, int yy, int zz, ItemStack[] itemToDrop)
  {
    for (ItemStack item : itemToDrop) {
      if ((!theWorld.isRemote) && (theWorld.getGameRules().getGameRuleBooleanValue("doTileDrops")) && (!theWorld.restoringBlockSnapshots))
      {
        ...
        theWorld.spawnEntityInWorld(entityitem);
      }
    }
  }
}

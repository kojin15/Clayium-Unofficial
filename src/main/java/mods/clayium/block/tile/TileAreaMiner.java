  ...
  implements IClayLaserMachine, IAxisAlignedBBContainer, IExternalControl
{
  ...
  public void initParamsByTier(int tier)
  {
    ...
    if (tier >= 10)
    {
      this.inventoryX = 4;this.inventoryY = 2;
    }
    switch (tier)
    {
    case 6: 
    ...
    else
    {
      for (int x = this.xCoord - 2; x <= this.xCoord + 2; x++) {
        for (int y = this.yCoord - 2; y <= this.yCoord + 2; y++) {
          ...
          {
            TileEntity tile = UtilBuilder.safeGetTileEntity(this.worldObj, x, y, z);
    ...
    case 4: 
      ...
      else if (consumeClayEnergy(c))
      {
        ...
        for (int count = 0; count < max; count++)
        {
          ...
          boolean isFluid = (FluidRegistry.lookupFluidForBlock(block) != null) || ((block.getMaterial() instanceof MaterialLiquid));
          double hardness = isFluid ? 1.0D : block.getBlockHardness(this.worldObj, this.miningX, this.miningY, this.miningZ);
          long i = (this.progressPerJob * (0.1D + hardness));
          if (block == Blocks.air) {
            i = 0L;
          }
          ...
          if (flag.booleanValue())
          {
            ...
            int fortune = 0;boolean silkTouch = false;
  ...
  {
    this.laserEnergy = ((this.laserEnergy + laser.getEnergy()));
    return true;
  }

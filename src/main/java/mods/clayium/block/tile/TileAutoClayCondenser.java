  ...
  extends TileClayMachines
{
  ...
  public void updateEntity()
  {
    ...
    for (int i = 0; i < num.length; i++) {
      this.clayEnergy += Math.pow(10.0D, i) * num[i];
    }
  }
  ...
  {
    if (this.containerItemStacks[20] == null) {
      this.machineConsumingEnergy = ((0.0F * this.multConsumingEnergy));
    }
    if (consumeClayEnergy(this.machineConsumingEnergy))
    {
      if (this.containerItemStacks[20] == null)
      {
        this.machineTimeToCraft = ((1.0F * this.multCraftTime));
  ...
  {
    return (super.canExtractItem(slot, itemstack, side)) && (getTierOfCompressedClay(itemstack) >= getTierOfCompressedClay(this.containerItemStacks[21]));
  }

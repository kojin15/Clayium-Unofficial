  ...
  extends TileClayContainerTiered
{
  ...
  public void initParamsByTier(int tier)
  {
    ...
    case 4: 
      ...
      this.autoExtractInterval = (this.autoInsertInterval = 8); break;
    case 5: 
      this.maxAutoInsertDefault = (this.maxAutoExtractDefault = 4);
      this.autoExtractInterval = (this.autoInsertInterval = 4);
      this.multConsumingEnergy = 0.0F; break;
    case 6: 
      this.maxAutoInsertDefault = (this.maxAutoExtractDefault = 16);
      this.autoExtractInterval = (this.autoInsertInterval = 2);
      this.multCraftTime = 0.05F; break;
    case 7: 
      this.maxAutoInsertDefault = (this.maxAutoExtractDefault = 1);
      this.autoExtractInterval = (this.autoInsertInterval = 1);
      this.multCraftTime = 0.05F;this.multConsumingEnergy = 4.0F;this.numAutomation = 16; break;
    case 8: 
      this.maxAutoInsertDefault = (this.maxAutoExtractDefault = 16);
      this.autoExtractInterval = (this.autoInsertInterval = 1);
      this.multCraftTime = 0.05F;this.multConsumingEnergy = 16.0F;this.numAutomation = 64; break;
    case 9: 
      this.maxAutoInsertDefault = (this.maxAutoExtractDefault = 'ɀ');
      this.autoExtractInterval = (this.autoInsertInterval = 1);
      this.multCraftTime = 0.05F;this.multConsumingEnergy = 64.0F;this.numAutomation = 256; break;
    default: 
      this.maxAutoInsertDefault = (this.maxAutoExtractDefault = 1);
  ...
  public void proceedCraft()
  {
    ...
    if (this.succeedConsumingCE)
    {
      boolean flag = true;
      for (int i = 0; i < 9; i++) {
        if (this.containerItemStacks[(24 + i)] != null) {
          flag = false;
        }
      }
      if (flag) {
      ...
      if (this.machineCraftTime >= timeToCraft)
      {
        ...
        for (int i = 0; i < 9; i++) {
          if (this.containerItemStacks[(24 + i)] != null)
          {
            ItemStack container = this.containerItemStacks[(24 + i)].getItem().getContainerItem(this.containerItemStacks[(24 + i)]);
            if (container != null) {
              UtilTransfer.produceItemStack(container, this.containerItemStacks, 34, 43, 64);
            }
            this.containerItemStacks[(24 + i)].stackSize -= 1;
            if (this.containerItemStacks[(24 + i)].stackSize <= 0) {
              this.containerItemStacks[(24 + i)] = null;
            }
          }
        }

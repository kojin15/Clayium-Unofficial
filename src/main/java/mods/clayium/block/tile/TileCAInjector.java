  ...
  extends TileCAMachines
{
  ...
  public void initParamsByTier(int tier)
  {
    ...
    {
    case 9: 
      this.caFactorExponent = 0.2D; break;
    case 10: 
      this.caFactorExponent = 0.9D; break;
    case 11: 
  ...
  protected boolean canCraft(ItemStack[] materials)
  {
    ...
    if (!this.containerItemStacks[2].isItemEqual(itemstacks[0])) {
      return false;
    }
  ...
  public boolean canProceedCraft()
  {
    ...
    {
      ItemStack[] itemstacks = { this.containerItemStacks[3], this.containerItemStacks[4] };
  ...
  public void proceedCraft()
  {
    ...
    if (consumeClayEnergy(this.machineConsumingEnergy))
    {
      if ((this.containerItemStacks[3] == null) && (this.containerItemStacks[4] == null))
      {
        ...
        this.machineTimeToCraft = ((this.recipe.getTime(itemstacks, method, this.baseTier) * getCraftTimeMultiplier() * this.multCraftTime));
        int[] consumedStackSize = this.recipe.getConsumedStackSize(itemstacks, method, this.baseTier);
        for (int i = 0; i < perm.length; i++)
        {
          this.containerItemStacks[(i + 3)] = this.containerItemStacks[perm[i]].splitStack(consumedStackSize[i]);
      ...
      if (this.machineCraftTime >= this.machineTimeToCraft)
      {
        ...
        for (int i = 0; i < perm.length; i++) {
          if (this.containerItemStacks[(i + 3)].stackSize -= consumedStackSize[i] <= 0) {
            this.containerItemStacks[(i + 3)] = null;
          }
        }

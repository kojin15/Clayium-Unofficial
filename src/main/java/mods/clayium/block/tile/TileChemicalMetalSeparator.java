  ...
  extends TileClayMachines
{
  ...
  protected boolean canCraft(ItemStack material)
  {
    ...
    for (int i = 0; i < resultlist.size(); i++) {
      if (UtilTransfer.canProduceItemStack((ItemStack)resultlist.get(i), this.containerItemStacks, 1, 17, getInventoryStackLimit()) < ((ItemStack)resultlist.get(i)).stackSize) {
        return false;
      }
    }
    return true;
  ...
  {
    if (this.containerItemStacks[17] == null) {
      this.machineConsumingEnergy = ((baseConsumingEnergy * this.multConsumingEnergy));
    }
    if (consumeClayEnergy(this.machineConsumingEnergy))
    {
      if (this.containerItemStacks[17] == null)
      {
        this.machineTimeToCraft = ((baseCraftTime * this.multCraftTime));

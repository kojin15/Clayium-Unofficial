  ...
  extends TileClayMachines
{
  ...
  protected boolean canCraft(ItemStack material)
  {
    ...
    for (int i = 0; i < Math.min(this.resultSlotNum, itemstacks.length); i++) {
      ...
      {
        if (!this.containerItemStacks[(i + 1)].isItemEqual(itemstacks[i])) {
          return false;
        }
  ...
  public void proceedCraft()
  {
    int method = 0;
    if (this.containerItemStacks[5] == null) {
      this.machineConsumingEnergy = (((float)this.recipe.getEnergy(new ItemStack[] { this.containerItemStacks[0] }, method, this.baseTier) * this.multConsumingEnergy));
    }
    if (consumeClayEnergy(this.machineConsumingEnergy))
    {
      if (this.containerItemStacks[5] == null)
      {
        this.machineTimeToCraft = (((float)this.recipe.getTime(new ItemStack[] { this.containerItemStacks[0] }, method, this.baseTier) * this.multCraftTime));

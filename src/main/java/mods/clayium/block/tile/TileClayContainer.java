  ...
  implements ISidedInventory, IInventoryFlexibleStackLimit, IOverridableBlock
{
  ...
  public int[] insertRoutes = { -1, -1, -1, -1, -1, -1 };
  public int[] extractRoutes = { -1, -1, -1, -1, -1, -1 };
  ...
  public boolean isItemValidForSlotDefault(int slot, ItemStack itemstack)
  {
    if (slot == this.clayEnergySlot) {
      return isEnergyClayValid(slot, itemstack);
    }
  ...
  public boolean checkInsertionList(int slot, ItemStack itemstack)
  {
    ...
    {
      int[] slots = (int[])this.listSlotsInsert.get(i);
  ...
  {
    return (acceptEnergyClay()) && (hasClayEnergy(itemstack)) && ((this.containerItemStacks[slot] == null) || (this.containerItemStacks[slot].stackSize < this.clayEnergyStorageSize));
  }
    ...
    for (int i = 0; i < this.listSlotsInsert.size(); i++) {
      if (this.insertRoutes[(UtilDirection.direction2Side(front, side) - 6)] == i) {
        for (int j = 0; j < ((int[])this.listSlotsInsert.get(i)).length; j++) {
          flag[((int[])this.listSlotsInsert.get(i))[j]] = 1;
        }
      }
    }
    for (int i = 0; i < this.listSlotsExtract.size(); i++) {
      if (this.extractRoutes[(UtilDirection.direction2Side(front, side) - 6)] == i) {
        for (int j = 0; j < ((int[])this.listSlotsExtract.get(i)).length; j++) {
          flag[((int[])this.listSlotsExtract.get(i))[j]] = 1;
        }
      }
    }
    int c = 0;
  ...
  public boolean canInsertItemUnsafe(int slot, ItemStack itemstack, int route)
  {
    ...
    int[] slots = (int[])this.listSlotsInsert.get(route);
    for (int j = 0; j < slots.length; j++) {
      if ((slots[j] == slot) && (isItemValidForSlot(slot, itemstack))) {
        return true;
      }
    }
    return false;
  ...
  public boolean canExtractItemUnsafe(int slot, ItemStack itemstack, int route)
  {
    ...
    int[] slots = (int[])this.listSlotsExtract.get(route);
    for (int j = 0; j < slots.length; j++) {
      if (slots[j] == slot) {
        return true;
      }
  ...
  public void doAutoExtractFromSide(int side, int front)
  {
    ...
    if ((route >= 0) && (route < this.listSlotsInsert.size())) {
      UtilTransfer.extract(this, (int[])this.listSlotsInsert.get(route), UtilDirection.getOrientation(front), UtilDirection.getSide(side), (this.maxAutoExtract != null) && (route < this.maxAutoExtract.length) && (this.maxAutoExtract[route] >= 0) ? this.maxAutoExtract[route] : this.maxAutoExtractDefault, new Object[0]);
    }
  }
  ...
  public void doAutoInsertToSide(int side, int front)
  {
    ...
    if ((route >= 0) && (route < this.listSlotsExtract.size())) {
      UtilTransfer.insert(this, (int[])this.listSlotsExtract.get(route), UtilDirection.getOrientation(front), UtilDirection.getSide(side), (this.maxAutoInsert != null) && (route < this.maxAutoInsert.length) && (this.maxAutoInsert[route] >= 0) ? this.maxAutoInsert[route] : this.maxAutoInsertDefault, new Object[0]);
    }
  }
  ...
  public int getItemUseMode(ItemStack itemStack, EntityPlayer player)
  {
    ...
    if (itemStack.getItem() == CItems.itemClayRollingPin) {
      return 0;
    }
    if (itemStack.getItem() == CItems.itemClaySlicer) {
      return 1;
    }
    if (itemStack.getItem() == CItems.itemClaySpatula) {
      return 2;
    }
    if ((itemStack.getItem() instanceof ItemFilterTemp)) {
      return 10;
    }
    if (itemStack.getItem() == CItems.itemRawClayCraftingTools) {
      return 11;
    }
    if (itemStack.getItem() == CItems.itemClayWrench) {
      return 20;
    }
  ...
  public void useItemFromSide(ItemStack itemStack, EntityPlayer player, int side, int mode)
  {
    ...
    case 10: 
      ...
      IItemFilter itemfilter = (IItemFilter)filter.getItem();
      ...
      else if (this.filters[side] != null)
      {
        ...
        ItemStack appliedFilter = this.filters[side];
        list = new ArrayList();
        ((IItemFilter)appliedFilter.getItem()).addFilterInformation(appliedFilter, player, (List)list, true);
    ...
    case 11: 
      if (this.filters[side] != null)
      {
        ...
        for (list = list.iterator(); ((Iterator)list).hasNext();)
        {
    ...
    case 20: 
      if (this.worldObj != null)
      {
        ...
        if (((block instanceof ClayContainer)) && 
          (((ClayContainer)block).metaMode != 0))
        {
          int front = ((ClayContainer)block).getFront(this.worldObj, this.xCoord, this.yCoord, this.zCoord);
          int direction = UtilDirection.side2Direction(front, side);
          if ((((ClayContainer)block).metaMode != 1) || ((direction != 0) && (direction != 1))) {
    ...
    case 30: 
      if (this.worldObj != null)
      {
        Block block1 = this.worldObj.getBlock(this.xCoord, this.yCoord, this.zCoord);
    ...
    case 31: 
      ...
      break;
    case 90: 
      if ((this.worldObj != null) && (acceptInterfaceSync()))
      {
  ...
  public boolean consumeClayEnergy(long energy)
  {
    ...
    {
      if (produceClayEnergy()) {
        return consumeClayEnergy(energy);
      }
      return false;
    }

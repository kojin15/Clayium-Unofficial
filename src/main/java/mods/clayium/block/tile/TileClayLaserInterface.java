...
public class TileClayLaserInterface
  ...
  implements IClayLaserMachine, ISynchronizedInterface
{
  ...
  protected boolean syncCoreBlock(int x, int y, int z)
  {
    ...
    {
      TileEntity te;
      if ((((te = UtilBuilder.safeGetTileEntity(getCoreWorld(), coreX, coreY, coreZ)) instanceof IClayLaserMachine)) && ((x != 0) || (y != 0) || (z != 0)))
      {
        this.synced = true;
        return true;
      }
  ...
  public boolean irradiateClayLaser(ClayLaser laser, UtilDirection direction)
  {
    if (syncCoreBlock())
    {
      IClayLaserMachine te = (IClayLaserMachine)UtilBuilder.safeGetTileEntity(getWorldObj(), this.xCoord + this.coreBlockX, this.yCoord + this.coreBlockY, this.zCoord + this.coreBlockZ);

  ...
  implements IExternalControl
{
  ...
  public void updateEntity()
  {
    ...
    for (UtilDirection direction : directions)
    {
      ...
      if ((canOverclock()) && ((block instanceof IOverclocker))) {
        this.overclockTotalFactor *= ((IOverclocker)block).getOverclockFactor(this.worldObj, this.xCoord + direction.offsetX, this.yCoord + direction.offsetY, this.zCoord + direction.offsetZ);
      }
      if ((block instanceof IClayContainerModifier)) {
        ((IClayContainerModifier)block).modifyClayContainer(this.worldObj, this.xCoord + direction.offsetX, this.yCoord + direction.offsetY, this.zCoord + direction.offsetZ, this);
      }
    }
  ...
  private RecipeCondition oldRecipeCondition;
  private RecipeResult oldRecipeResult;
  private RecipeCondition[] oldRecipeConditions = new RecipeCondition[65];
  private RecipeResult[] oldRecipeResults = new RecipeResult[65];
  private boolean[] refresh = new boolean[65];
  
  protected RecipeCondition getRecipeCondition(ItemStack material, int baseTier)
  {
    if (material == null) {
      return null;
    }
    ...
    if (this.refresh[n] == 0)
    {
      RecipeCondition newRecipeCondition = this.recipe.getRecipeConditionFromInventory(new ItemStack[] { material }, 0, baseTier);
  ...
  protected RecipeResult getRecipeResult(ItemStack material, int baseTier)
  {
    if (material == null) {
      return null;
    }
  ...
  {
    RecipeResult result = getRecipeResult(material, baseTier);
  ...
  {
    RecipeCondition cond = getRecipeCondition(material, baseTier);
    if (cond == null) {
      return 0;
    }
  ...
  protected ItemStack getResult(ItemStack material, int baseTier)
  {
    ...
    return results[0] == null ? null : (results == null) || (results.length == 0) ? null : results[0].copy();
  }
  ...
  protected boolean canCraft(ItemStack material)
  {
    ...
    if (!this.containerItemStacks[1].isItemEqual(itemstack)) {
      return false;
    }
  ...
  {
    if (this.containerItemStacks[2] == null) {
      this.machineConsumingEnergy = (((float)getEnergy(this.containerItemStacks[0], this.baseTier) * this.multConsumingEnergy));
    }
    if (consumeClayEnergy(this.machineConsumingEnergy))
    {
      if (this.containerItemStacks[2] == null)
      {
        this.machineTimeToCraft = (((float)getTime(this.containerItemStacks[0], this.baseTier) * this.multCraftTime));

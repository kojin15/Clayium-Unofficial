...
public class TileClayMarker
  extends TileGeneric
  implements IAxisAlignedBBProvider, IAxisAlignedBBContainer
{
  ...
  public void activate()
  {
    if (!this.worldObj.isRemote)
    {
      ...
      if ((this.xx != 0) || (this.yy != 0) || (this.zz != 0) || (this.state == 1))
      {
        ...
        this.state = (++this.state >= 5 ? 2 : this.state <= 1 ? 3 : this.state);
      }
      else
      {
        this.aabb = null;
  ...
  {
    return this.worldObj.getBlock(this.xCoord + xx, this.yCoord + yy, this.zCoord + zz) instanceof ClayMarker;
  }

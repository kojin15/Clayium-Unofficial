  ...
  implements IPANComponent
{
  ...
  public void proceedCraft()
  {
    ...
    this.machineConsumingEnergy = ((100000.0F * this.multConsumingEnergy));
    double rest = result0.consumption - this.machineCraftTime * this.machineConsumingEnergy;
    this.machineTimeToCraft = ((result0.consumption / this.machineConsumingEnergy));
    long consumption = rest > this.machineConsumingEnergy ? this.machineConsumingEnergy : rest;
      ...
      if ((this.containerItemStacks[3] == null) && (this.containerItemStacks[4] == null)) {
        for (int i = 0; i < 2; i++)
        {
          ...
          if (!UtilItemStack.areTypeEqual(this.containerItemStacks[i], getAntimatter())) {
            this.containerItemStacks[i].stackSize += 1;
          }

  ...
  implements ISynchronizedInterface
{
  ...
  public static String[] stateSequence = { "None", "Emit if idle", "Emit if work scheduled", "Emit if doing work", "Do work", "Do not work", "Start work", "Stop work", "Do work once" };
  ...
  protected boolean syncCoreBlock(int x, int y, int z)
  {
    ...
    {
      TileEntity te;
      if ((((te = UtilBuilder.safeGetTileEntity(getCoreWorld(), coreX, coreY, coreZ)) instanceof IExternalControl)) && ((x != 0) || (y != 0) || (z != 0)))
      {
        this.synced = true;
        return true;
      }

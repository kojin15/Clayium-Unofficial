  ...
  extends TileClayMachines
{
  ...
  public void initParams()
  {
    ...
    this.listSlotsInsert.clear();this.listSlotsExtract.clear();
  ...
  {
    this.initCraftTime = ((float)(Math.pow(10.0D, this.acceptableTier + 1) * (this.baseCraftTime - 1.0F) / (this.baseCraftTime * (Math.pow(this.baseCraftTime, this.acceptableTier) - 1.0D)) / (ClayiumCore.multiplyProgressionRateF(5000.0F) / 20.0F)));
    if (tier >= 6)
    {
      ...
      this.initCraftTime = ((float)(Math.pow(10.0D, this.acceptableTier + 1) * (this.baseCraftTime - 1.0F) / (this.baseCraftTime * (Math.pow(this.baseCraftTime, this.acceptableTier) - 1.0D)) / (ClayiumCore.multiplyProgressionRateF(50000.0F) / 20.0F)));
    }
    if (tier >= 7)
    {
      ...
      this.initCraftTime = ((float)(Math.pow(10.0D, this.acceptableTier + 1) * (this.baseCraftTime - 1.0F) / (this.baseCraftTime * (Math.pow(this.baseCraftTime, this.acceptableTier) - 1.0D)) / (ClayiumCore.multiplyProgressionRateF(3000000.0F) / 20.0F)));
    }
  }
  ...
  {
    return UtilTransfer.canProduceItemStack(getCompressedClay(tier + 1), this.containerItemStacks, 1, 2, getInventoryStackLimit()) >= 1;
  }
  ...
  public boolean canProceedCraft()
  {
    ...
    return this.containerItemStacks[2] == null ? canCraft(getTierOfCompressedClay(this.containerItemStacks[0])) : canCraft(getTierOfCompressedClay(this.containerItemStacks[2]));
  }
  ...
  public void proceedCraft()
  {
    ...
    {
      this.machineTimeToCraft = ((Math.pow(this.baseCraftTime, getTierOfCompressedClay(this.containerItemStacks[0])) * this.multCraftTime));
    ...
    this.clayEnergy = ((Math.pow(10.0D, getTierOfCompressedClay(this.containerItemStacks[2]) + 1) * this.machineCraftTime / this.machineTimeToCraft));
  ...
  {
    return (getTierOfCompressedClay(itemstack) >= 0) && (getTierOfCompressedClay(itemstack) <= this.acceptableTier);
  }

  ...
  extends TileClayContainer
{
  ...
  public int getStackSizeInSlot(int slot)
  {
    ...
    int[] map = new int[this.extractSlotNum + this.insertSlotNum];int p = 0;
  ...
  public int getStackSizeInSlot(int slot, int containerStackSize, int containerMaxStorageSize, int itemMaxStackSize, int extractSlotNum, int insertSlotNum)
  {
    ...
    if ((slot >= extractSlotNum) && (slot < extractSlotNum + insertSlotNum))
    {
      ...
      return slot0 == filledSlots ? insertAreaStackSize - filledSlots * maxStackSize : slot0 < filledSlots ? maxStackSize : 0;
    }
    return slot == filledSlots ? stackSize - filledSlots * maxStackSize : slot < filledSlots ? maxStackSize : 0;
  }
  ...
  public ItemStack decrStackSize(int slot, int size)
  {
    ...
    if (this.alternativeItemStacks[slot] == null)
    {
      int stackSize = getStackSizeInSlot(slot);
      int resSize = size >= stackSize ? stackSize : size;
      if ((this.containerItemStacks[0] == null) || (resSize == 0)) {
        return null;
      }
      ItemStack res = this.containerItemStacks[0].splitStack(resSize);
  ...
  {
    debug("setInventorySlotContents(" + slot + "," + itemstack + "@" + (itemstack == null ? "n" : Integer.valueOf(itemstack.hashCode())) + ")");
    if (slot >= this.extractSlotNum + this.insertSlotNum + 1)
    {
  ...
  public void setInventorySlotContentsUnsafe(int slot, ItemStack itemstack)
  {
    ...
    debug("done setInventorySlotContentsUnsafe(" + slot + "," + itemstack + "@" + (itemstack == null ? "n" : Integer.valueOf(itemstack.hashCode())) + ")");
  }
  ...
  {
    return (slot >= this.insertSlotNum + this.extractSlotNum + 1) || (((this.containerItemStacks[0] == null) || (UtilItemStack.areTypeEqual(itemstack, this.containerItemStacks[0]))) && ((this.containerItemStacks[1] == null) || (checkFilterSlot(itemstack, this.containerItemStacks[1]))));
  }

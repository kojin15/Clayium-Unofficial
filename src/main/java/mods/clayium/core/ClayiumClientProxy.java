  ...
  extends ClayiumCommonProxy
{
  ...
  public void registerRenderer()
  {
    ...
    IItemRenderer renderer = new ItemDamagedRenderer();
    for (Iterator<Entry<CMaterial, ItemDamaged>> iterator = CMaterials.materialMap.entrySet().iterator(); iterator.hasNext();)
    {
      Entry<CMaterial, ItemDamaged> entry = (Entry)iterator.next();
      MinecraftForgeClient.registerItemRenderer((Item)entry.getValue(), renderer);
    }
    for (Iterator<Entry<CShape, ItemDamaged>> iterator = CMaterials.shapeMap.entrySet().iterator(); iterator.hasNext();)
    {
      entry = (Entry)iterator.next();
      MinecraftForgeClient.registerItemRenderer((Item)entry.getValue(), renderer);
    }
    Entry<CShape, ItemDamaged> entry;
    if (ClayiumCore.cfgEnableFluidCapsule)
    {
      iterator = CItems.itemsCapsule;entry = iterator.length;
      for (Entry<CShape, ItemDamaged> localEntry1 = 0; localEntry1 < entry; localEntry1++)
      {
        ItemCapsule item = iterator[localEntry1];
  ...
  {
    boolean flag = false;int s = 5;
    for (int i = 0; i < 9; i++)
    {
      ItemStack item = player.inventory.mainInventory[i];
      if ((item != null) && ((item.getItem() instanceof ItemBlock)) && 
        (((ItemBlock)item.getItem()).field_150939_a == CBlocks.blockOverclocker))
      {
        flag |= true;
        switch (item.getItemDamage())
        {
        case 10: 
          s = Math.min(s, 3); break;
        case 11: 
          s = Math.min(s, 2); break;
        case 12: 
          s = Math.min(s, 1); break;
        case 13: 
          s = Math.min(s, 0);
        }
      }

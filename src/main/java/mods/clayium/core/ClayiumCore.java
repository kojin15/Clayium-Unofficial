...
import mods.clayium.item.ItemCapsule.RecipeCapsulePackaging;
import mods.clayium.item.ItemCapsule.RecipeCapsuleUnpackaging;
...
public class ClayiumCore
{
  ...
  public static final CreativeTabs creativeTabClayium = new CreativeTab("Clayium");
  ...
  public static Map<IntegrationID, Boolean> cfgModIntegration = new HashMap();
  private EntityPlayer playerEntity;
  
  public static enum IntegrationID
  {
    NEI("NotEnoughItems", "NotEnoughItems"),  MINE_TWEAKER("MineTweaker3", "MineTweaker3"),  MULTI_PART("ForgeMultipart", "ForgeMultipart"),  IC2("IC2", "IndustrialCraft2"),  EIO("EnderIO", "EnderIO"),  TF("ThermalFoundation", "ThermalFoundation"),  FFM("Forestry", "Forestry"),  AE2("appliedenergistics2", "AppliedEnergistics2"),  TIC("TConstruct", "TinkersConstruct"),  PR_EX("ProjRed|Exploration", "ProjectRedExploration"),  MEK("Mekanism", "Mekanism"),  BR("BigReactors", "BigReactors"),  GC("GalacticraftCore", "Galacticraft"),  FZ("factorization", "Factorization"),  GT("gregtech", "GregTech"),  MET("Metallurgy", "Metallurgy"),  SS2("SextiarySector", "SextiarySector"),  MAPLE("mod_ecru_MapleTree", "MapleTree"),  TOFU("TofuCraft", "TofuCraft"),  MISC("Misc");
    ...
    {
      this.isMod = true;this.modId = modId;this.configId = configId;
    }
    
    private IntegrationID(String configId)
    {
      this.isMod = false;this.modId = "";this.configId = configId;
    }
    
    public boolean enabled()
    {
      Boolean b = (Boolean)ClayiumCore.cfgModIntegration.get(this);
  ...
  public void preInit(FMLPreInitializationEvent event)
  {
    ...
    ForgeChunkManager.setForcedChunkLoadingCallback(INSTANCE, new ClayChunkLoaderCallback());
    if (!cfgUtilityMode)
    {
  ...
  public void init(FMLInitializationEvent event)
  {
    ...
    if (IntegrationID.MULTI_PART.loaded())
    {
      ...
      for (BlockDamaged[] blocks : new BlockDamaged[][] { blocks0, (BlockDamaged[])MetalBlock.metalBlockMap.values().toArray(new BlockDamaged[0]) })
      {
        BlockDamaged block;
        for (block : blocks) {
          for (Integer i : block.getAvailableMetadata()) {
            UtilMultipart.registerMicroBlock(block, i.intValue());
          }
        }
      }
    }
  ...
  public void postInit(FMLPostInitializationEvent event)
  {
    ...
    if (cfgEnableFluidCapsule)
    {
      ...
      GameRegistry.addRecipe(new ItemCapsule.RecipeCapsulePackaging());
      GameRegistry.addRecipe(new ItemCapsule.RecipeCapsuleUnpackaging());
      FluidContainerRegistry.FluidContainerData data;
      for (data : FluidContainerRegistry.getRegisteredFluidContainerData()) {
        for (ItemCapsule capsule : ItemCapsule.getAllCapsules()) {
          if ((capsule.getCapacity() == data.fluid.amount) && (!(data.filledContainer.getItem() instanceof ItemCapsule))) {
            try
            {
              ...
              {
                CRecipes.recipeFluidTransferMachine.addRecipe(new ItemStack[] { data.emptyContainer.copy(), new ItemStack(capsule, 1, UtilFluid.getFluidID(data.fluid)) }, 0, 5, new ItemStack[] { data.filledContainer.copy() }, CRecipes.e(5), 1L);
                CRecipes.recipeFluidTransferMachine.addRecipe(new ItemStack[] { data.filledContainer.copy() }, 0, 5, new ItemStack[] { data.emptyContainer.copy(), new ItemStack(capsule, 1, UtilFluid.getFluidID(data.fluid)) }, CRecipes.e(5), 1L);
              }
              else
              {
                CRecipes.recipeFluidTransferMachine.addRecipe(new ItemStack[] { data.filledContainer.copy() }, 0, 5, new ItemStack[] { new ItemStack(capsule, 1, UtilFluid.getFluidID(data.fluid)) }, CRecipes.e(5), 1L);
              }
            }
  ...
  public static int multiplyProgressionRateI(int a)
  {
    ...
    return a > 0 ? 1 : a < 0 ? -1 : r != 0 ? r : 0;
  }
  
  public static long multiplyProgressionRateL(long a)
  {
    long r = (a * cfgProgressionRate);
    return a > 0L ? 1L : a < 0L ? -1L : r != 0L ? r : 0L;
  }

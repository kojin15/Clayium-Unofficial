    ...
    if (!this.worldObj.isRemote)
    {
      if ((getThrower() != null) && ((getThrower() instanceof EntityPlayerMP)))
      {
        ...
        if ((entityplayermp.playerNetServerHandler.func_147362_b().isChannelOpen()) && (entityplayermp.worldObj == this.worldObj))
        {
          ...
          {
            this.worldObj.playSoundAtEntity(getThrower(), "mob.endermen.portal", 1.0F, 1.0F);
            ...
            double dx = 0.0D;double dy = 0.0D;double dz = 0.0D;int c = 0;
            double x;
            double y;
            double z;
            do
            {
              x = Math.floor(event.targetX + dx);y = Math.floor(event.targetY + dy);z = Math.floor(event.targetZ + dz);
              dx = this.rand.nextGaussian() * c / 10.0D;dy = this.rand.nextGaussian() * c / 10.0D;dz = this.rand.nextGaussian() * c / 10.0D;
            ...
            int length = (int)Math.sqrt((event.targetX - getThrower().posX) * (event.targetX - getThrower().posX) + 
              (event.targetY - getThrower().posY) * (event.targetY - getThrower().posY) + 
              (event.targetZ - getThrower().posZ) * (event.targetZ - getThrower().posZ));

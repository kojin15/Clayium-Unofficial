...
import mods.clayium.util.UtilCoordinate.Vec3RYP;
...
public class RayTraceMemory
{
  ...
  public static RayTraceMemory getFromNBT(NBTTagCompound tag)
  {
    ...
    return new RayTraceMemory(Vec3.createVectorHelper(tag.getDouble("posX"), tag.getDouble("posY"), tag.getDouble("posZ")), Vec3.createVectorHelper(tag.getDouble("hitX"), tag.getDouble("hitY"), tag.getDouble("hitZ")), tag.getInteger("hitRX"), tag.getInteger("hitRY"), tag.getInteger("hitRZ"), tag.getByte("hitSide"));
  }
  ...
  {
    int hX = MathHelper.floor_double(hitPos.xCoord);int eX = MathHelper.floor_double(entityPos.xCoord);
    int hY = MathHelper.floor_double(hitPos.yCoord);int eY = MathHelper.floor_double(entityPos.yCoord);
    int hZ = MathHelper.floor_double(hitPos.zCoord);int eZ = MathHelper.floor_double(entityPos.zCoord);
  ...
  public static boolean onPlayerRightClick(EntityPlayer player, World world, ItemStack itemstack, int x, int y, int z, int side, Vec3 hitVec)
  {
    ...
    if ((itemstack != null) && 
      ...
      (itemstack.getItem().onItemUseFirst(itemstack, player, world, x, y, z, side, f, f1, f2)))
    {
  ...
  public static boolean canPlaceItemIntoWorld(ItemBlock itemBlock, World world, int x, int y, int z, int side, EntityPlayer player, ItemStack itemStack)
  {
    ...
    x = coord[0];y = coord[1];z = coord[2];side = coord[3];
  ...
  public static MovingObjectPosition rayTraceEntity(World world, Vec3 pos, Vec3 look, double reach, Class entityClass, IEntitySelector selector)
  {
    ...
    float f1 = 1.0F;
    List list = world.selectEntitiesWithinAABB(entityClass, AxisAlignedBB.getBoundingBox(pos.xCoord, pos.yCoord, pos.zCoord, pos.xCoord, pos.yCoord, pos.zCoord).addCoord(normalizedLook.xCoord * reach, normalizedLook.yCoord * reach, normalizedLook.zCoord * reach).expand(f1, f1, f1), selector);

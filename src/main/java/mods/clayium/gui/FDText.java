  ...
  extends FDWithHandler<FDText<T>, T>
{
  ...
  {
    List<FDTextHandler<T>> list = new ArrayList();
    if (preHandlers != null) {
      list.addAll(preHandlers);
    }
    FDTextHandler<T> aHandler = new FDTextHandler();
    aHandler.handleX = (aHandler.handleY = aHandler.handleColor = aHandler.handleMaximumLength = 1);
    aHandler.defaultX = x;aHandler.defaultY = y;aHandler.defaultColor = color;aHandler.defaultMaximumLength = maxLength;
    list.add(aHandler);
    if (postHandlers != null) {
      list.addAll(postHandlers);
    }
    return new FDText(list);
  }
  
  public FDText<T> getFDText(FDTextHandler<T> preHandler, int x, int y, int color, int maxLength)
  {
    return getFDText(Arrays.asList((FDTextHandler[])new FDTextHandler[] { preHandler }), x, y, color, maxLength, null);
  }

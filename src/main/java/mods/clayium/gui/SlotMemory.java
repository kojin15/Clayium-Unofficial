  ...
  extends SlotWithTexture
{
  ...
  public boolean canTakeStack(EntityPlayer player)
  {
    ...
    if ((playerstack != null) && ((playerstack.getItem() instanceof IItemFilter)) && (slotstack != null) && 
      ((slotstack.getItem() instanceof IItemFilter)))
    {
      IItemFilter playerfilter = (IItemFilter)playerstack.getItem();
      if (playerfilter.isCopy(playerstack))
      {
        playerstack = slotstack.copy();playerfilter = (IItemFilter)playerstack.getItem();
        playerstack = playerfilter.setCopyFlag(playerstack);
        if (!player.worldObj.isRemote)
        {
          player.addChatMessage(new ChatComponentText("Copied " + slotstack.getDisplayName()));

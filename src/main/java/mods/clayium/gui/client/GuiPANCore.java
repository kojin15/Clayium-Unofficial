  ...
  extends GuiClayEnergyTemp
{
      ...
      (this.conversionList != null))
    {
      ...
      for (TilePANCore.ItemStackWithEnergy item : this.conversionList)
      {
        ...
        if ((item.itemstack != null) && (flag)) {
          this.mc.thePlayer.addChatMessage(new ChatComponentText(item.toString()));
        }
      }
    }
    if (id == 5) {
  ...
  public void drawGuiContainerForegroundLayer(int mouseX, int mouseZ)
  {
    ...
    if (this.conversionList != null)
    {
      ...
      int ix = Math.max(this.tableWidth / 16, 1);int iy = Math.max(this.tableHeight / 16, 1);
      if (this.detail) {
        ix = 1;
      }
      int count = 0;
  ...
  public GuiPANCore(ContainerTemp container, TilePANCore tile, Block block)
  {
    ...
    this.comp = new TilePANCore.ItemStackComparator();
    this.comparators = new ReversibleComparator[3];
    
    this.comparators[0] = new ReversibleComparator(this.comp);
    this.comparators[1] = new ReversibleComparator(new CostComparator());
    this.comparators[2] = new ReversibleComparator(new ConsumptionComparator());

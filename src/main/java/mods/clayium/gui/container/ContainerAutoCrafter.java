  ...
  extends ContainerTemp
{
  ...
  public void setMachineInventorySlots(InventoryPlayer player)
  {
    ...
    for (int j = 0; j < 3; j++) {
      for (int i = 0; i < 3; i++)
      {
        SlotWithTexture slot = new SlotWithTexture(tile, i + j * 3, i * 18 + (this.machineGuiSizeX - 54) / 2 + 1, j * 18 + 18);
    ...
    for (int j = 0; j < 3; j++) {
      for (int i = 0; i < 3; i++) {
        addMachineSlotToContainer(new SlotMemory(tile, i + j * 3 + 15, i * 18 + 5, j * 18 + 18, RectangleTexture.SmallSlotFilterTexture));
      }
    }
    for (int j = 0; j < 3; j++) {
      for (int i = 0; i < 2; i++) {
        addMachineSlotToContainer(new SlotResultWithTexture(tile, i + j * 2 + 9, i * 18 + (this.machineGuiSizeX - 36) - 5, j * 18 + 18));
      }
    }
  }

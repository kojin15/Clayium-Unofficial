  ...
  extends ContainerClayMachines
{
  ...
  {
    addMachineSlotToContainer(new SlotWithTexture(this.tile, 0, 44, 35, RectangleTexture.LargeSlotTexture));
    for (int i = 0; i < this.resultSlotNum; i++) {
      addMachineSlotToContainer(new SlotFurnace(player.player, this.tile, i + 1, 116, 35 + 18 * i - 9 * (this.resultSlotNum - 1)));
    }
  }
}

  ...
  extends ContainerTemp
{
  ...
  public static ContainerClayCraftingTable newInstance(InventoryPlayer player, TileClayContainer inventory, Block block)
  {
    ...
    {
      TileEntity tile = UtilDirection.getTileEntity(inventory.getWorldObj(), inventory.xCoord, inventory.yCoord, inventory.zCoord, direction);
      if ((tile instanceof TileEntityChest)) {
        return new ContainerClayCraftingTable(player.player, new IInventory[] { inventory, (IInventory)tile }, block, new int[] { 0, 0 }, new int[] { 3, 9 }, new int[] { 3, 3 }, new int[] { 30, 8 }, new int[] { 17, 73 }, 124, 35);
      }
    }
    return new ContainerClayCraftingTable(player.player, new IInventory[] { inventory }, block, new int[] { 0 }, new int[] { 3 }, new int[] { 3 }, new int[] { 30 }, new int[] { 17 }, 124, 35);
  }
  ...
  {
    super(player.inventory, new InventoriesClayCraftingTable(inventories, starts, widths, heights, xs, ys, resultX, resultY), block, new Object[0]);
    ...
    for (int i = 0; i < inv.inventories.length; i++) {
      for (int y = 0; y < inv.heights[i]; y++) {
        for (int x = 0; x < inv.widths[i]; x++) {
          addMachineSlotToContainer(new SlotWithTexture(this, inv.inventories[i], inv.starts[i] + x + y * inv.widths[i], inv.xs[i] + x * 18, inv.ys[i] + y * 18, RectangleTexture.SmallSlotTexture));
        }
      }
    }
    this.resultSlot = addMachineSlotToContainer(new SlotCCrafting(player.player, this, inv.inventories, inv.starts, inv.widths, inv.heights, new InventoryCraftResult(), 0, inv.resultX, inv.resultY));
  }
  ...
  {
    return (mergeItemStack(itemstack1, this.widths[0] * this.heights[0], this.resultSlot, false)) || (mergeItemStack(itemstack1, 0, this.widths[0] * this.heights[0], false));
  }
  ...
  {
    ((Slot)this.inventorySlots.get(this.resultSlot)).inventory.setInventorySlotContents(0, CraftingManager.getInstance().findMatchingRecipe(new InventoryCraftingInTile(this.widths[0], this.heights[0], this.inventories[0], this.starts[0]), this.world));
  }

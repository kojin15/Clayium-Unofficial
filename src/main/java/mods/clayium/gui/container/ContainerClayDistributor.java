      ...
      for (int ii = 0; ii < tile.inventoryColonyX; ii++) {
        for (int j = 0; j < tile.inventoryY; j++) {
          for (int i = 0; i < tile.inventoryX; i++) {
            addMachineSlotToContainer(new Slot(tile, i + tile.inventoryX * (j + tile.inventoryY * (ii + tile.inventoryColonyX * jj)), i * 18 + ii * (tile.inventoryX * 18 + 2) + (this.machineGuiSizeX - (tile.inventoryX * tile.inventoryColonyX * 18 + (tile.inventoryColonyX - 1) * 2)) / 2 + 1, j * 18 + jj * (tile.inventoryY * 18 + 2) + 18));
          }
        }
      }
    }

  ...
  extends ContainerTemp
{
  ...
  {
    super(player.inventory, null, null, new Object[0]);
    this.player = player;
  }
  ...
  public void setMachineInventorySlots(InventoryPlayer player)
  {
    for (int j = 0; j < inventoryY; j++) {
      for (int i = 0; i < inventoryX; i++) {
        addMachineSlotToContainer(new SlotMemory(this.tile, i + j * inventoryX, i * 18 + (this.machineGuiSizeX - 18 * inventoryX) / 2, j * 18 + 18));
      }
    }
  }
  ...
  public void addPlayerInventorySlots(InventoryPlayer player)
  {
    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < 9; j++) {
        addSlotToContainer(new Slot(player, j + i * 9 + 9, this.playerSlotOffsetX + 8 + j * 18, this.playerSlotOffsetY + 12 + i * 18));
      }
    }
    for (i = 0; i < 9; i++) {
      if (player.currentItem == i)
      {
        addSlotToContainer(new Slot(player, i, this.playerSlotOffsetX + 8 + i * 18, this.playerSlotOffsetY + 70)
        {
  ...
  protected void initParameters(InventoryPlayer player)
  {
    ...
    this.tile = new InventoryInItemStack(player.getCurrentItem(), "Items", inventoryX * inventoryY);

  ...
  extends ItemTiered
{
  ...
  {
    player.worldObj.playSoundAtEntity(player, "game.neutral.hurt.fall.small", 0.6F, 5.0F / (itemRand.nextFloat() * 0.7F + getDamage(stack, player) * per + 1.0F));
    float v = getInitialVelocity(stack, player) * per;
    if (v >= 6.0F)
    {
      player.worldObj.playSoundAtEntity(player, "fireworks.launch", 0.01F * (v - 6.0F), 1.0F);
      player.worldObj.playSoundAtEntity(player, "random.explode", 0.01F * (v - 6.0F), 6.0F / (v + 2.0F) + 0.2F);
    }
    spawnEntity(stack, player, per, critical);
    if (!this.infinity) {
      stack.damageItem((int)(per * getChargeTime(stack, player)) + 1, player);
    }
  }
  ...
  public void spawnEntity(ItemStack stack, EntityPlayer player, float per, boolean critical)
  {
    if (!player.worldObj.isRemote) {
      player.worldObj.spawnEntityInWorld(new EntityClayBall(player.worldObj, player, 
  ...
  public void onUsingTick(ItemStack stack, EntityPlayer player, int count)
  {
    if (isCharger(stack, player))
    {
      ...
      if (c == 0) {
        player.worldObj.playSoundAtEntity(player, "random.click", 0.5F, 1.3F);
      }
      if (c == getChargeTime(stack, player))
      {
        player.worldObj.playSoundAtEntity(player, "note.hat", 0.5F, 0.6F);
        player.worldObj.playSoundAtEntity(player, "mob.enderdragon.hit", 0.2F, 2.5F);
      }
    }
  }
}

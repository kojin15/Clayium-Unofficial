...
public class ItemGadgetHolder
  extends ItemTiered
{
  protected static List<IItemGadget> gadgetList = new ArrayList();
  protected static Map<Entity, Map<IItemGadget, List<ItemStack>>> gadgetMapRemote = new HashMap();
  protected static Map<Entity, Map<IItemGadget, List<ItemStack>>> gadgetMapServer = new HashMap();
  ...
  protected static Map<IItemGadget, List<ItemStack>> getGadgetMap(Entity entity, boolean isRemote)
  {
    ...
    return (Map)gadgetMap.get(entity);
  }
  ...
  public void onUpdate(ItemStack itemStack, World world, Entity entity, int slot, boolean isCurrentItem)
  {
    ...
    for (Object localObject = getGadgetList().iterator(); ((Iterator)localObject).hasNext();)
    {
      gadget = (IItemGadget)((Iterator)localObject).next();
      list = (List)gadgetMap.get(gadget);
    ...
    localObject = items;IItemGadget gadget = localObject.length;
    for (List<ItemStack> list = 0; list < gadget; list++)
    {
      ItemStack item = localObject[list];
  ...
  public static void updateGadget(Entity entity, boolean isRemote)
  {
    ...
    {
      List<ItemStack> list = (List)gadgetMap.get(gadget);

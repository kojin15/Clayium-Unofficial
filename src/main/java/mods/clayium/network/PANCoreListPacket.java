...
import mods.clayium.block.tile.TilePANCore.ItemStackComparator;
import mods.clayium.block.tile.TilePANCore.ItemStackWithEnergy;
...
{
  public int windowId;
  public Set<ItemStackWithEnergy> ingreds;
  public Set<ItemStackWithEnergy> prohibiteds;
  ...
  public static void writeSetToBuffer(Set<ItemStackWithEnergy> set, ByteBuf buf)
    throws IOException
  {
    if (set != null)
    {
      buf.writeInt(set.size());
      for (ItemStackWithEnergy item : set)
      {
  ...
  public static Set<ItemStackWithEnergy> readSetFromBuffer(ByteBuf buf)
    throws IOException
  {
    Set<ItemStackWithEnergy> set = new TreeSet(new ItemStackComparator());
    ...
    for (int i = 0; i < size; i++)
    {
      ...
      set.add(new ItemStackWithEnergy(itemstack, cost, consumption));
    }
    return set;
  }
  
  public static void writeItemStackToBuffer(ItemStack p_150788_1_, ByteBuf buf)
    throws IOException
  {
    if (p_150788_1_ == null)
    {
  ...
  public static void writeNBTTagCompoundToBuffer(NBTTagCompound p_150786_1_, ByteBuf buf)
    throws IOException
  {
    if (p_150786_1_ == null)
    {

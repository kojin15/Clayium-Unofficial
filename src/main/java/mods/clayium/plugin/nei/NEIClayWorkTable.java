...
{
  protected static ItemStack[][] tools = { null, { null }, { null }, { new ItemStack(CItems.itemClayRollingPin) }, { new ItemStack(CItems.itemClaySlicer), new ItemStack(CItems.itemClaySpatula) }, { new ItemStack(CItems.itemClaySpatula) }, { new ItemStack(CItems.itemClaySlicer), new ItemStack(CItems.itemClaySpatula) } };
    ...
    extends TemplateRecipeHandler.CachedRecipe
  {
    ...
    {
      List<PositionedStack> res = new ArrayList();
      res.add(this.input);
      if (this.tool != null) {
        res.add(this.tool);
      }
    ...
    implements Comparator<TemplateRecipeHandler.CachedRecipe>
  {
    ...
    public int compare(TemplateRecipeHandler.CachedRecipe a1, TemplateRecipeHandler.CachedRecipe b1)
    {
      ...
      int[] aa = getMaxId(Arrays.asList(new PositionedStack[] { recipeCacher.access$000(a) }));
      int[] bb = getMaxId(Arrays.asList(new PositionedStack[] { recipeCacher.access$000(b) }));
      if (aa[0] > bb[0]) {
        return 1;
      }
      if (bb[0] > aa[0]) {
  ...
  public void loadCraftingRecipes(String outputId, Object... results)
  {
    ...
    if ((outputId == null) || (outputId.equals(getOverlayIdentifier()))) {
      for (iterator = ClayWorkTableRecipes.smelting().kneadingList.entrySet().iterator(); iterator.hasNext();)
      {
  ...
  {
    for (Iterator<Entry> iterator = ClayWorkTableRecipes.smelting().kneadingList.entrySet().iterator(); iterator.hasNext();)
    {
      Entry entry = (Entry)iterator.next();
      ItemStack input = (ItemStack)((Map)entry.getKey()).get("Material");
      int method = ((Integer)((Map)entry.getKey()).get("ButtonId")).intValue();

...
public class UtilAdvancedTools
{
  ...
  public static int onBlockDestroyed(ItemStack itemstack, World world, Block block, int x, int y, int z, EntityLivingBase entity)
  {
    ...
    if ((!world.isRemote) && 
      ...
      for (Vec3 coord : getHarvestedCoordListInSafe(itemstack, world, x, y, z, (EntityPlayer)entity)) {
        ...
        {
          UtilBuilder.harvestBlockAndDropItems(world, (int)coord.xCoord, (int)coord.yCoord, (int)coord.zCoord, true, 
          
            (world.getBlock((int)coord.xCoord, (int)coord.yCoord, (int)coord.zCoord).canSilkHarvest(world, (EntityPlayer)entity, (int)coord.xCoord, (int)coord.yCoord, (int)coord.zCoord, world.getBlockMetadata((int)coord.xCoord, (int)coord.yCoord, (int)coord.zCoord))) && 
            (EnchantmentHelper.getSilkTouchModifier((EntityPlayer)entity)), 
            EnchantmentHelper.getFortuneModifier((EntityPlayer)entity));
          damage++;
        }
      }
    }
    if ((world.isRemote) && 
      ...
      ClayiumCore.packetDispatcher.sendToServer(new ClaySteelPickaxePacket(world, x, y, z));
    }
    return damage;
  }
  ...
  public void breakSpeedSubscriber(PlayerEvent.BreakSpeed event)
  {
    ...
    if ((item != null) && ((item.getItem() instanceof IAdvancedTool)))
    {
      ...
      {
        HashMap<String, Integer> map = (HashMap)forceList.get(event.entityPlayer);
        forceList.remove(event.entityPlayer);
        if ((((Integer)map.get("x")).intValue() == event.x) && (((Integer)map.get("y")).intValue() == event.y) && (((Integer)map.get("z")).intValue() == event.z) && 
          (((Integer)map.get("d")).intValue() == event.entityPlayer.worldObj.provider.dimensionId))
        {
      ...
      if (firstCall == true)
      {
        ...
        if ((event.newSpeed >= 24.0F) && 
          ...
          ClayiumCore.packetDispatcher.sendToServer(new ClaySteelPickaxePacket(event.entityPlayer.worldObj, event.x, event.y, event.z));
        }
        firstCall = true;
      }
  ...
  public static Vec3[] getInverse(Vec3[] v)
  {
    ...
    return new Vec3[] {Vec3.createVectorHelper((v[1].yCoord * v[2].zCoord - v[2].yCoord * v[1].zCoord) / d, (v[2].yCoord * v[0].zCoord - v[0].yCoord * v[2].zCoord) / d, (v[0].yCoord * v[1].zCoord - v[1].yCoord * v[0].zCoord) / d), Vec3.createVectorHelper((v[1].zCoord * v[2].xCoord - v[2].zCoord * v[1].xCoord) / d, (v[2].zCoord * v[0].xCoord - v[0].zCoord * v[2].xCoord) / d, (v[0].zCoord * v[1].xCoord - v[1].zCoord * v[0].xCoord) / d), Vec3.createVectorHelper((v[1].xCoord * v[2].yCoord - v[2].xCoord * v[1].yCoord) / d, (v[2].xCoord * v[0].yCoord - v[0].xCoord * v[2].yCoord) / d, (v[0].xCoord * v[1].yCoord - v[1].xCoord * v[0].yCoord) / d) };
  }
}

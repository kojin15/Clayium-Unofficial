...
public class UtilBuilder
{
  ...
  public static ItemStack createStackedBlock(Block block, int metadata)
  {
    if (createStackedBlock == null)
    {
      ...
      if (classBlock != null) {
        try
        {
          createStackedBlock = classBlock.getDeclaredMethod("createStackedBlock", new Class[] { Integer.TYPE });
        }
        catch (NoSuchMethodException e)
        {
          try
          {
            createStackedBlock = classBlock.getDeclaredMethod("func_149644_j", new Class[] { Integer.TYPE });
          }
          catch (NoSuchMethodException e1)
          {
            try
            {
              createStackedBlock = classBlock.getDeclaredMethod("j", new Class[] { Integer.TYPE });
            }
  ...
  public static ItemStack getItemBlock(World theWorld, int xx, int yy, int zz)
  {
    ...
    for (ItemStack dropitem : dropitems) {
      ...
      {
        ItemStack res = dropitem.copy();res.stackSize = 1;
        return res;
      }
    }
    for (ItemStack dropitem : dropitems) {
  ...
  public static boolean placeBlockAt(Block block, ItemStack stack, World world, int x, int y, int z, int metadata, boolean useFakePlayer)
  {
    ...
    {
      if (useFakePlayer) {
        block.onBlockPlacedBy(world, x, y, z, UtilPlayer.getFakePlayer(null, world, x, y, z), stack);
      }
      block.onPostBlockPlaced(world, x, y, z, metadata);
    }
    return true;

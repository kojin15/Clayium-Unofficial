...
public class UtilPlayer
{
  ...
  public void FOVUpdateEventSubscriber(FOVUpdateEvent event)
  {
    ...
    if ((event.entity.isUsingItem()) && ((stack.getItem() instanceof ClayShooter)))
    {
      ...
      if (item.isCharger(stack, event.entity))
      {
        ...
        float u = i / time;float k = t * u - 10.0F;
        if (k < 0.0F) {
          k = 0.0F;
        }
  ...
  {
    return !dataMapGet(player).containsKey(key) ? null : !dataMapContainsKey(player) ? null : dataMapGet(player).get(key);
  }
  
  public static void setPlayerInstantData(EntityPlayer player, String key, Object value)
  {
    if (!dataMapContainsKey(player)) {
      dataMapPut(player, new HashMap());
    }
  ...
  {
    return (HashMap)getPlayerInstantDataMap(isRemote(player)).put(player, value);
  }
  
  public static HashMap<String, Object> dataMapGet(EntityPlayer player)
  {
    return (HashMap)getPlayerInstantDataMap(isRemote(player)).get(player);
  }
  ...
  public static EntityPlayer getFakePlayer(String id)
  {
    ...
    if ((id != null) || (defaultPlayer == null))
    {
      ...
      ret.playerNetServerHandler = new NetHandlerPlayServerFakePlayer(ret);
      ret.eyeHeight = 0.0F;
    }
    if (id == null)
    {
    ...
    extends NetHandlerPlayServer
  {
    ...
    {
      super(new NetworkManagerFakePlayer(), player);
    }

...
{
  static abstract interface IInventorySelector
  {
    public abstract IInventory getSelectedInventory();
    
    public abstract boolean selectInventoryToInsertTo(TileEntity paramTileEntity, ForgeDirection paramForgeDirection);
    
    public abstract int[] getSlotToInsertTo(ForgeDirection paramForgeDirection);
    
    public abstract boolean selectInventoryToExtractFrom(TileEntity paramTileEntity, ForgeDirection paramForgeDirection);
    
    public abstract int[] getSlotToExtractFrom(ForgeDirection paramForgeDirection);
  }
  ...
  public static int transfer(IInventory from, IInventory to, int[] fromSlots, int[] toSlots, int fromSide, int toSide, int maxTransfer)
  {
    ...
    try
    {
      for (int fromSlot : fromSlots)
      {
        ...
        if ((fromItem != null) && (fromItem.stackSize > 0) && ((fromSided == null) || 
          (fromSided.canExtractItem(fromSlot, fromItem, fromSide))))
        {
          int maxMove;
          if (fromItem.isStackable()) {
            for (int toSlot : toSlots)
            {
              ...
              if ((toItem != null) && (toItem.stackSize > 0) && ((toSided == null) || 
                (toSided.canInsertItem(toSlot, fromItem, toSide))) && 
                (fromItem.isItemEqual(toItem)) && (ItemStack.areItemStackTagsEqual(toItem, fromItem)))
              {
                ...
                maxMove = Math.max(Math.min(maxSize - toItem.stackSize, Math.min(maxTransfer, fromItem.stackSize)), 0);
          ...
          if (fromItem.stackSize > 0) {
            ...
            {
              ItemStack toItem = to.getStackInSlot(toSlot);
              if ((toItem == null) && (to.isItemValidForSlot(toSlot, fromItem)) && ((toSided == null) || 
                (toSided.canInsertItem(toSlot, fromItem, toSide))))
              {
  ...
  public static boolean canProduceItemStacks(ItemStack[] itemstacks, ItemStack[] inventory, int i, int j, int inventoryStackLimit)
  {
    ...
    for (int k = 0; k < copyItemstacks.length; k++) {
      if (produceItemStack(copyItemstacks[k], copyInventory, i, j, inventoryStackLimit) != null) {
        return false;
      }
    }
    return true;
    ...
    for (int k = i; k < j; k++) {
      if (inventory[k] != null) {
          ...
          (ItemStack.areItemStackTagsEqual(inventory[k], itemstack)))
        {
          ...
          stackSize -= n;inventory[k].stackSize -= n;
          if (inventory[k].stackSize <= 0) {
            inventory[k] = null;
          }
        }
      }
  ...
  private static int[] capacities = { 1000, 125, 25, 5, 1 };
  ...
  public static int insertToTank(TileEntity fromTe, int[] fromSlots, ForgeDirection direction, int maxTransfer)
  {
    ...
    {
      ItemStack fromItem = from.getStackInSlot(fromSlot);
      if ((fromItem != null) && (fromItem.stackSize > 0) && ((fromItem.getItem() instanceof ItemCapsule)) && ((fromSided == null) || 
        (fromSided.canExtractItem(fromSlot, fromItem, fromSide))))
      {
        ...
        if (fluidStack != null)
        {
          ...
          if (fromItem.hasTagCompound()) {
            fluidStack.tag = fromItem.getTagCompound();
          }
          ...
          for (int i = 0; i < capsules.length; i++)
          {
            if (amount >= capacities[i])
            {
              ...
              if (fluidStack.tag != null) {
                fromItem.setTagCompound((NBTTagCompound)fluidStack.tag.copy());
              }
              for (int toSlot : toSlots)
              {
                ItemStack toItem = to.getStackInSlot(toSlot);
  ...
  public static void initFluidParams()
  {
    ...
    capsuleMap = new HashMap();
    
    int[] capacities = new int[aCapsules.length];
    for (int i = 0; i < aCapsules.length; i++)
    {
      capacities[i] = aCapsules[i].getCapacity();
      capacityMap.put(aCapsules[i], Integer.valueOf(capacities[i]));
      capsuleMap.put(Integer.valueOf(capacities[i]), aCapsules[i]);
    }
    Arrays.sort(capacities);
    sortedCapacities = new int[capacities.length];
    for (int i = 0; i < sortedCapacities.length; i++) {
    ...
    {
      sortedCapsules[i] = ((ItemCapsule)capsuleMap.get(Integer.valueOf(sortedCapacities[i])));
  ...
  public static void sortFluid(ItemStack[] inventory, int[] slots)
  {
    ...
    Map<CapsuleKey, List<Integer>> capsuleSlots = new HashMap();
    Map<CapsuleKey, Integer> amounts = new HashMap();
    
    boolean slotsIsNull = slots == null;int iMax = slotsIsNull ? inventory.length : slots.length;
    ItemStack itemstack = null;int slot = -1;Item item = null;
    for (int i = 0; i < iMax; i++)
    {
      ...
      else if (((item = itemstack.getItem()) instanceof ItemCapsule))
      {
        CapsuleKey key = new CapsuleKey(itemstack);
        List<Integer> list = (List)capsuleSlots.get(key);
        int amount = 0;
        if (list == null)
        {
          list = new ArrayList();
        ...
        {
          amount = ((Integer)amounts.get(key)).intValue();
        }
        list.add(Integer.valueOf(slot));
        
        amount += itemstack.stackSize * ((Integer)capacityMap.get(item)).intValue();
        amounts.put(key, Integer.valueOf(amount));
      }
    }
    for (Entry<CapsuleKey, List<Integer>> entry : capsuleSlots.entrySet())
    {
      CapsuleKey key = (CapsuleKey)entry.getKey();
      int amount = ((Integer)amounts.get(key)).intValue();
      List<Integer> slots1 = (List)entry.getValue();
      ...
      if (stackNum <= slots1.size() + emptySlots.size())
      {
        boolean tagIsNull = key.tag == null;
        int slotsSize = slots1.size();int emptySlotsSize = emptySlots.size();
        int slotMax = slotsSize + emptySlotsSize;int slot = 0;
        ...
        for (int sloti = 0; sloti < slotMax; sloti++)
        {
          ...
          if (sloti < slotsSize) {
            slot = ((Integer)slots1.get(sloti)).intValue();
          } else {
            slot = ((Integer)emptySlots.get(sloti - slotsSize)).intValue();
          }
          if (i >= stackSizes.length)
          {
            inventory[slot] = null;
            if (sloti < slotsSize) {
              toAdd.add(Integer.valueOf(slot));
            }
          }
          else
          {
            if (sloti >= slotsSize) {
              toRemove.add(Integer.valueOf(slot));
            }
            if (stackSizes[i] > sortedStackLimits[i])
            {
              inventory[slot] = new ItemStack(sortedCapsules[i], sortedStackLimits[i], key.damage);
              if (!tagIsNull) {
                inventory[slot].setTagCompound(key.tag);
              }
              stackSizes[i] -= sortedStackLimits[i];
            }
            else
            {
              inventory[slot] = new ItemStack(sortedCapsules[i], stackSizes[i], key.damage);
              if (!tagIsNull) {
                inventory[slot].setTagCompound(key.tag);
              }
              i++;
  ...
  public static int fillFluid(ItemStack[] inventory, int[] slots, FluidStack resource, boolean doFill)
  {
    ...
    List<Integer> emptySlots = new ArrayList();
    List<Integer> capsuleSlots = new ArrayList();
    int amount = 0;
    
    boolean slotsIsNull = slots == null;int iMax = slotsIsNull ? inventory.length : slots.length;
    ItemStack itemstack = null;int slot = -1;Item item = null;boolean resourceHasTag = resource.tag != null;
    for (int i = 0; i < iMax; i++)
    {
      if (slotsIsNull)
      {
      ...
      else if ((((item = itemstack.getItem()) instanceof ItemCapsule)) && (resourceFluidId == itemstack.getItemDamage()) && (((!resourceHasTag) && 
        (!itemstack.hasTagCompound())) || ((resourceHasTag) && (resource.tag.equals(itemstack.getTagCompound())))))
      {
        capsuleSlots.add(Integer.valueOf(slot));
        amount += itemstack.stackSize * ((Integer)capacityMap.get(item)).intValue();
      }
    }
    ...
    int slotsSize = capsuleSlots.size();int emptySlotsSize = emptySlots.size();
    int slotMax = slotsSize + emptySlotsSize;int slot = 0;
    int i = 0;
    for (int sloti = 0; sloti < slotMax; sloti++)
    {
      ...
      if (sloti < slotsSize) {
        slot = ((Integer)capsuleSlots.get(sloti)).intValue();
      } else {
        slot = ((Integer)emptySlots.get(sloti - slotsSize)).intValue();
      }
      if (i >= stackSizes.length)
      ...
      else if (stackSizes[i] > sortedStackLimits[i])
      {
        inventory[slot] = new ItemStack(sortedCapsules[i], sortedStackLimits[i], resourceFluidId);
        if (!tagIsNull) {
          inventory[slot].setTagCompound(resource.tag);
        }
        stackSizes[i] -= sortedStackLimits[i];
      }
      else
      {
        inventory[slot] = new ItemStack(sortedCapsules[i], stackSizes[i], resourceFluidId);
        if (!tagIsNull) {
          inventory[slot].setTagCompound(resource.tag);
        }
        i++;
  ...
  public static FluidStack drainFluid(ItemStack[] inventory, int[] slots, int maxDrain, boolean doDrain)
  {
      ...
      else if (((item = itemstack.getItem()) instanceof ItemCapsule))
      {
        if (ret == null)
        {
          ...
          if (resourceFluid != null)
          {
            ...
            if (resourceHasTag) {
              ret.tag = itemstack.getTagCompound();
            }
          }
          else
          {
        ...
        if ((ret != null) && (resourceFluidId == itemstack.getItemDamage()) && (((!resourceHasTag) && 
          (!itemstack.hasTagCompound())) || ((resourceHasTag) && (ret.tag.equals(itemstack.getTagCompound())))))
        {
          capsuleSlots.add(Integer.valueOf(slot));
          amount += itemstack.stackSize * ((Integer)capacityMap.get(item)).intValue();
        }
    ...
    for (int sloti = 0; sloti < slotMax; sloti++)
    {
      ...
      else if (stackSizes[i] > sortedStackLimits[i])
      {
        inventory[slot] = new ItemStack(sortedCapsules[i], sortedStackLimits[i], resourceFluidId);
        if (resourceHasTag) {
          inventory[slot].setTagCompound(ret.tag);
        }
        stackSizes[i] -= sortedStackLimits[i];
      }
      else
  ...
  public static boolean canFillFluid(ItemStack[] inventory, int[] slots, Fluid fluid)
  {
    ...
    for (int i = 0; i < iMax; i++)
    {
      ...
      else if ((((item = itemstack.getItem()) instanceof ItemCapsule)) && (itemstack.getItemDamage() == fluidId))
      {
        amount += itemstack.stackSize * ((Integer)capacityMap.get(item)).intValue();
        slotNum++;
      }
  ...
  public static boolean canDrainFluid(ItemStack[] inventory, int[] slots, Fluid fluid)
  {
    ...
    boolean slotsIsNull = slots == null;int iMax = slotsIsNull ? inventory.length : slots.length;
    ItemStack itemstack = null;int slot = -1;
    for (int i = 0; i < iMax; i++)
    {
      if (slotsIsNull)
      {
  ...
  public static FluidTankInfo[] getTankInfo(ItemStack[] inventory, int[] slots)
  {
    ...
    Map<CapsuleKey, Integer> amounts = new HashMap();
    Map<CapsuleKey, Integer> slotNums = new HashMap();
    int emptySlotNum = 0;
    ...
    for (int i = 0; i < iMax; i++)
    {
      ...
      else if (((item = itemstack.getItem()) instanceof ItemCapsule))
      {
        ...
        {
          int amount = 0;int slotNum = 0;
          if (amounts.containsKey(key))
          {
            amount = ((Integer)amounts.get(key)).intValue();
            slotNum = ((Integer)slotNums.get(key)).intValue();
          }
          amount += itemstack.stackSize * ((Integer)capacityMap.get(item)).intValue();
          amounts.put(key, Integer.valueOf(amount));
          slotNum++;
          slotNums.put(key, Integer.valueOf(slotNum));
        }
      }
    }
    ...
    int i = 0;
    for (Entry<CapsuleKey, Integer> entry : amounts.entrySet())
    {
      CapsuleKey key = (CapsuleKey)entry.getKey();
      ...
      ret[i] = new FluidTankInfo(fluid, (((Integer)slotNums.get(key)).intValue() + emptySlotNum) * sortedCapacities[0] * sortedStackLimits[0]);
      i++;
    }
    return ret;
  }

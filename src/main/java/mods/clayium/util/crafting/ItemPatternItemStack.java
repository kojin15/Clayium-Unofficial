  ...
  implements IItemPattern
{
  ...
  {
    if (itemstack != null) {
      this.items = new ArrayList(Arrays.asList(new ItemStack[] { itemstack.copy() }));
    } else {
      this.items = new ArrayList(Arrays.asList(new ItemStack[] { null }));
    }
  }
  ...
  public int getStackSize(ItemStack itemstack)
  {
    for (ItemStack item : this.items)
    {
      ...
      else if (UtilItemStack.areItemDamageEqual(itemstack, item)) {
        return Math.max(item.stackSize, 1);
      }
    }
    return 1;
  ...
  public ItemStack isSimple()
  {
    ...
    ItemStack ret = (ItemStack)this.items.get(0);
